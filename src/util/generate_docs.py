...
def write_doc_files(sensor_tables: dict, docs_location: Path):
    ...
    for sensor in sensor_tables:
        ...
        if old_doc not in old_doc_files:
            ...
            file_beginning.append(
                f'        <a class="btn btn-primary" target="_blank" href="../../csv/{sensor}-sensors-mappings-enterprise.csv">'
            )
            file_beginning.append(
            ...
            file_beginning.append("")
            file_beginning.append(
                f'        <a class="btn btn-primary" target="_blank" href="../../stix/{sensor}-mappings-enterprise.json">'
            )
            file_beginning.append(
            ...
            file_beginning.append("")
            file_beginning.append(
                f'        <a class="btn btn-primary" target="_blank" href="https://mitre-attack.github.io/attack-navigator/#layerURL=https://center-for-threat-informed-defense.github.io/sensor-mappings-to-attack/navigator/{sensor}-heatmap.json">'
            )
            file_beginning.append(
...
def csv_to_tables(attack_types, mappings_location):
    ...
    for attack_type in attack_types:
        ...
        csv_files = [
            filename for filename in current_dir.iterdir() if filename.is_file()
        ]
...
def extract_row_data(row: dict):
    # Unwrap multi-line inputs
    description = " ".join(filter(None, row["EVENT DESCRIPTION"].splitlines()))

    result = [
        f"  * - | **{row['EVENT ID']}**",
        f"      |",
        f"      | {description}",
        f"    - | **Data Source:** {row['ATT&CK DATA SOURCE']}",
        f"      | **Data Component:** {row['ATT&CK DATA COMPONENT']}",
    ]
...
def table_start(sensor):
    ...
    table_header.append(f"  :widths: 50 50")
    table_header.append("  :header-rows: 1")
    table_header.append("")
    table_header.append("  * - EVENT")
    table_header.append("    - ATT&CK MAPPING")
    table_header.append("")
    return table_header
...
def main():
    ...
    attack_types = [
        i
        for i in os.listdir(args.mappings_location)
        if Path(args.mappings_location, i).is_dir()
    ]
